    ...
    <script type="text/javascript">
		...
		function save() {
			...
			if (initModel == 'INSERT') {				
				transaction({
					id: 'INSERT',
					url: '<%=request.getContextPath()%>/console/executor/insert',
					jsonSet: jsonSet
				});
			} else {
				transaction({
					id: 'UPDATE',
					url: '<%=request.getContextPath()%>/console/executor/update',
					jsonSet: jsonSet
				});				
			}
		}
	...
	<div>
		<div id="form" style="width: auto;">
			<table width="100%" border="1" cellspacing="0" cellpadding="0" class="s_layout">
			    ...
			    <tr>
			    	<th><label id="executor_Name_Label">执行机器名称</label></th>
			    	<td><input type="text" id="executor_Name" maxlength="32"/></td>
					<th><label id="rpc_Type_Label">RPC类型</label></th>
			    	<td>
			    		<select id="rpc_Type" style="width: 83.5%;">
